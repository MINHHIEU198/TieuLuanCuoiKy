...
public class Kho {
	...
	public void findByType() {
		...
		HangHoaType type = HangHoaType.input();
		for (HangHoa hangHoa : list) {
			if (hangHoa.loai.equals(type)) li.add(hangHoa);
		}
		Kho.print(li);
	}
	...
	public void statistical() {
		int totalQuantity =0;
		double totalCost =0;
		int[] totalPerType = new int[HangHoaType.values().length];
		...
		for (HangHoa hangHoa : list) {
			...
			for (int i=0;i<3;i++) {
				if (hangHoa.getLoai().compareTo(HangHoaType.values()[i]) ==0) totalPerType[i]++;
			}
		}
		...
		for(int i=0;i<3;i++) {
			System.out.println("\t"+HangHoaType.values()[i]+ ": "+totalPerType[i]+" hàng");
		}
	}
